...
class TNCustomerVault {
  ...
  static updateAccount(params) {
    ...
    if (params.creditCard) {
      ...
      if (creditCard.hasOwnProperty('accountNumber')) {
        requestParams.ccnumber = creditCard.accountNumber;
      }
      if (creditCard.hasOwnProperty('expirationDate')) {
        requestParams.ccexp = creditCard.expirationDate;
      }
      if (creditCard.hasOwnProperty('cvv')) {
        requestParams.cvv = creditCard.cvv;
      }
    } else if (params.check) {
      const { check } = params;
      if (check.hasOwnProperty('routingNumber')) {
        requestParams.checkaba = check.routingNumber;
      }
      if (check.hasOwnProperty('accountNumber')) {
        requestParams.checkaccoung = check.accountNumber;
      }
      if (check.hasOwnProperty('nameOnAccount')) {
        requestParams.checkname = check.nameOnAccount;
      }
      if (check.hasOwnProperty('accountType')) {
        requestParams.account_type = check.accountType;
      }
      if (check.hasOwnProperty('accountHolderType')) {
        requestParams.account_holder_type = check.accountHolderType;
      }
    } else {
      throw new Error('TransNational[TNCustomerVault]: missing required parameter(s) - check or credit card information');
    }
